    ...
    try:
        ...
        if not fixtures:
            if page == 0:
                ...
                # No more pages — go back silently
        ...
        await query.edit_message_text(ERROR_TIMEOUT, parse_mode=ParseMode.MARKDOWN, reply_markup=back_to_menu_keyboard())
        ...
        await query.edit_message_text(ERROR_GENERAL, parse_mode=ParseMode.MARKDOWN, reply_markup=back_to_menu_keyboard())
...
async def multi_select_handler(
    ...
    if not fixtures:
        await query.edit_message_text(
            "⚠️ Session expired. Please go back and reload fixtures.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=back_to_menu_keyboard(),
        )
        return
...
async def toggle_fixture_handler(
    ...
    if fixture_id in selected_ids:
        ...
        if len(selected_ids) >= 5:
            # Can't double-answer here since it's already answered in callback_router
            # Just silently ignore — the UI already shows 5 checked
            return
